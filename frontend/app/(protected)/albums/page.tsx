...
export default function AlbumsPage() {
  ...
  async function handleRemoveFromAlbum(photoId: string) {
    if (!selected) return;
    ...
    return (
      <div>
        <button
          ...
          className="text-sv-text3 text-sm mb-4 hover:text-sv-text">
          ← Retour aux albums
        </button>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => handleDeleteAlbum(selected.id)}
          ...
          <div className="mb-5 border border-sv-border bg-sv-surface rounded-2xl p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-semibold">
                Choisir des photos à ajouter
              </span>
              <button
                onClick={() => setAdding(false)}
                className="text-sv-text3 text-xs hover:text-sv-text">
                Fermer
              </button>
            </div>
            {loadingPhotos && (
              <p className="text-sv-text3 text-sm">Chargement…</p>
            )}
            {!loadingPhotos && availablePhotos.length === 0 && (
              <p className="text-sv-text3 text-sm">
                Toutes vos photos sont déjà dans cet album.
              </p>
            )}
            <div
        ...
        <div className="[column-width:180px] sm:[column-width:200px] gap-x-3">
          {photos.map((photo, i) => (
            <button
              ...
              className="block w-full break-inside-avoid mb-3 rounded-xl overflow-hidden border border-[#20262f] aspect-square bg-sv-surface2 text-left">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={photo.url}
                alt={photo.originalName}
          ...
          {photos.length === 0 && !adding && (
            <p className="text-sv-text3 text-sm">
              Cet album est vide pour l&apos;instant.
            </p>
          )}
        </div>
        ...
        {current && (
          <PhotoDetailOverlay
            ...
            removeFromAlbumLabel="Retirer de l'album"
              ...
              <div className="flex items-center justify-between mb-1">
                ...
                <span
                  ...
                  style={{
                    ...
                    color: "var(--color-sv-accent)"
                  }}>
                  {album.photos?.[0]?.storageMode === "cloud"
                    ? "Cloud"
                    : "Souverain"}
                </span>
              </div>
              <div className="text-xs text-sv-text3">
